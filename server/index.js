...
async function runMigrations() {
  ...
  const migrations = [
    ...
    { name: 'get_parent_for_kiosk', sql: `DROP FUNCTION IF EXISTS public.get_parent_for_kiosk(TEXT, TEXT, UUID, UUID); DROP FUNCTION IF EXISTS public.get_parent_for_kiosk(TEXT, TEXT); DROP FUNCTION IF EXISTS public.get_parent_for_kiosk(TEXT); CREATE OR REPLACE FUNCTION public.get_parent_for_kiosk(p_search_val TEXT, p_pin TEXT, p_user_id UUID DEFAULT NULL, p_org_id UUID DEFAULT NULL) RETURNS TABLE (id UUID, first_name TEXT, last_name TEXT, phone TEXT) AS $$ BEGIN RETURN QUERY SELECT p.id, p.first_name, p.last_name, p.phone FROM public.profiles p WHERE (regexp_replace(p.phone, '\\D', '', 'g') ILIKE '%' || regexp_replace(p_search_val, '\\D', '', 'g') || '%' OR p.first_name ILIKE '%' || p_search_val || '%' OR p.last_name ILIKE '%' || p_search_val || '%') AND p.security_pin = p_pin AND (p_org_id IS NULL OR p.organization_id = p_org_id OR p.organization_id IS NULL) LIMIT 5; END; $$ LANGUAGE plpgsql SECURITY DEFINER;` },
    { name: 'get_children_for_kiosk', sql: `DROP FUNCTION IF EXISTS public.get_children_for_kiosk(UUID, TEXT, UUID, UUID); DROP FUNCTION IF EXISTS public.get_children_for_kiosk(UUID, TEXT); DROP FUNCTION IF EXISTS public.get_children_for_kiosk(UUID); CREATE OR REPLACE FUNCTION public.get_children_for_kiosk(p_parent_id UUID, p_pin TEXT, p_user_id UUID DEFAULT NULL, p_org_id UUID DEFAULT NULL) RETURNS TABLE (id UUID, first_name TEXT, last_name TEXT, age INTEGER, class_id UUID, parent_id UUID) AS $$ BEGIN RETURN QUERY SELECT c.id, c.first_name, c.last_name, c.age, c.class_id, c.parent_id FROM public.children c JOIN public.profiles p ON c.parent_id = p.id WHERE p.id = p_parent_id AND p.security_pin = p_pin AND (p_org_id IS NULL OR c.organization_id = p_org_id OR c.organization_id IS NULL); END; $$ LANGUAGE plpgsql SECURITY DEFINER;` },
