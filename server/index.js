...
async function runMigrations() {
  ...
  const migrations = [
    ...
    { name: 'get_staff_verification_status_rpc', sql: `CREATE OR REPLACE FUNCTION public.get_staff_verification_status(p_user_id UUID) RETURNS TABLE (user_id UUID, is_verified BOOLEAN, verification_status TEXT, verified_at TIMESTAMPTZ, verified_by UUID, documents_count INTEGER) AS $$ BEGIN RETURN QUERY SELECT p.id as user_id, COALESCE(ur.role IN ('admin', 'super_admin', 'staff', 'teacher'), false) as is_verified, COALESCE(ur.role, 'pending')::TEXT as verification_status, p.created_at as verified_at, p.id as verified_by, (SELECT COUNT(*)::INTEGER FROM public.staff_documents sd WHERE sd.staff_id = p.id) as documents_count FROM public.profiles p LEFT JOIN public.user_roles ur ON p.id = ur.user_id WHERE p.id = p_user_id; END; $$ LANGUAGE plpgsql SECURITY DEFINER;` },
    { name: 'get_pending_staff_verifications_rpc', sql: `CREATE OR REPLACE FUNCTION public.get_pending_staff_verifications() RETURNS TABLE (user_id UUID, first_name TEXT, last_name TEXT, email TEXT, role TEXT, created_at TIMESTAMPTZ, documents_count INTEGER) AS $$ BEGIN RETURN QUERY SELECT p.id as user_id, COALESCE(p.first_name, '')::TEXT, COALESCE(p.last_name, '')::TEXT, COALESCE(p.email, '')::TEXT, COALESCE(ur.role::TEXT, p.role::TEXT, 'staff')::TEXT, COALESCE(p.created_at, NOW()), (SELECT COUNT(*)::INTEGER FROM public.staff_documents sd WHERE sd.staff_id = p.id) as documents_count FROM public.profiles p LEFT JOIN public.user_roles ur ON p.id = ur.user_id WHERE COALESCE(ur.role::TEXT, p.role::TEXT) IN ('staff', 'teacher', 'volunteer', 'pending') ORDER BY p.created_at DESC; END; $$ LANGUAGE plpgsql SECURITY DEFINER;` },
