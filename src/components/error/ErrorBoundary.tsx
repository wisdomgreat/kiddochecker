      ...
      return (
        <div className="min-h-screen w-full flex items-center justify-center bg-[#fef2f2] p-6 relative overflow-hidden font-sans">
           ...
           <motion.div 
            ...
            className="w-full max-w-lg bg-card/90 backdrop-blur-2xl border border-white shadow-[0_48px_80px_-16px_rgba(225,29,72,0.1)] rounded-[4rem] p-12 text-center space-y-10 relative z-10"
          >
             ...
             <div className="space-y-4">
                <h2 className="text-4xl font-bold text-foreground tracking-tighter">System <span className="text-rose-600">Exception</span></h2>
             ...
             <div className="space-y-4">
               <Button onClick={this.handleRetry} className="h-16 w-full rounded-[2rem] bg-slate-900 hover:bg-black text-white font-bold text-xs uppercase tracking-[0.2em] shadow-2xl shadow-slate-200 transition-all active:scale-95 flex items-center justify-center gap-3">
                 <RefreshCw className="h-4 w-4" />
                 Restart Interface
               </Button>
               <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 flex items-center justify-center gap-2">
                 Session UUID <span className="text-foreground px-2 py-0.5 bg-slate-100 rounded-md font-mono">{Math.random().toString(36).substring(7).toUpperCase()}</span>
               </p>
             </div>
          </motion.div>
        </div>
