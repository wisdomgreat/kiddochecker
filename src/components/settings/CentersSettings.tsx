...
const CentersSettings = () => {
    ...
    return (
        <div className="space-y-10">
            ...
            <Card className="bg-slate-50 dark:bg-card/5 border-none rounded-[2rem] overflow-hidden">
                <CardContent className="p-8 flex items-center justify-between gap-6">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="text-xl font-bold text-foreground dark:text-white tracking-tight">Public Location Page</h3>
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between px-2">
                    <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-xl font-bold text-foreground dark:text-white tracking-tight">Managed Locations</h3>
                    </div>
                    <Button 
                ...
                {isAdding && (
                    <Card className="bg-card dark:bg-slate-900 border-none shadow-xl rounded-[2rem] overflow-hidden ring-2 ring-indigo-500/20">
                        ...
                        <CardContent className="p-8 pt-0 space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-2">
                                    ...
                                    <Input value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="e.g. Toronto Central Branch" className="h-12 rounded-xl bg-slate-50 dark:bg-card/5 border-none px-4" />
                                </div>
                                <div className="space-y-2">
                                    <Label className="text-xs font-bold text-slate-400">Street Address</Label>
                                    <Input value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})} placeholder="123 Church St" className="h-12 rounded-xl bg-slate-50 dark:bg-card/5 border-none px-4" />
                                </div>
                                <div className="space-y-2">
                                    <Label className="text-xs font-bold text-slate-400">City</Label>
                                    <Input value={formData.city} onChange={e => setFormData({...formData, city: e.target.value})} placeholder="Toronto" className="h-12 rounded-xl bg-slate-50 dark:bg-card/5 border-none px-4" />
                                </div>
                                <div className="space-y-2">
                                    <Label className="text-xs font-bold text-slate-400">State / Province</Label>
                                    <Input value={formData.state_province} onChange={e => setFormData({...formData, state_province: e.target.value})} placeholder="Ontario" className="h-12 rounded-xl bg-slate-50 dark:bg-card/5 border-none px-4" />
                                </div>
                                <div className="space-y-2">
                                    <Label className="text-xs font-bold text-slate-400">Phone</Label>
                                    <Input value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} placeholder="(555) 000-0000" className="h-12 rounded-xl bg-slate-50 dark:bg-card/5 border-none px-4" />
                                </div>
                                <div className="space-y-2">
                                    <Label className="text-xs font-bold text-slate-400">Email</Label>
                                    <Input value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} placeholder="contact@grace.ca" className="h-12 rounded-xl bg-slate-50 dark:bg-card/5 border-none px-4" />
                                </div>
                                <div className="space-y-2">
                                    <Label className="text-xs font-bold text-slate-400">Latitude</Label>
                                    <Input type="number" step="any" value={formData.latitude} onChange={e => setFormData({...formData, latitude: parseFloat(e.target.value)})} className="h-12 rounded-xl bg-slate-50 dark:bg-card/5 border-none px-4" />
                                </div>
                                <div className="space-y-2">
                                    <Label className="text-xs font-bold text-slate-400">Longitude</Label>
                                    <Input type="number" step="any" value={formData.longitude} onChange={e => setFormData({...formData, longitude: parseFloat(e.target.value)})} className="h-12 rounded-xl bg-slate-50 dark:bg-card/5 border-none px-4" />
                                </div>
                            </div>
                ...
                <div className="grid grid-cols-1 gap-4">
                    {isLoading ? (
                        ...
                        <Card key={center.id} className="p-6 rounded-[1.5rem] bg-card dark:bg-slate-900 border-none shadow-sm hover:shadow-md transition-all group">
                            <div className="flex items-center justify-between gap-6">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h4 className="font-bold text-foreground dark:text-white leading-tight">{center.name}</h4>
                    ...
                    {!isLoading && centers?.length === 0 && (
                        <div className="text-center py-20 bg-slate-50 dark:bg-card/5 rounded-[2rem] border-2 border-dashed border-slate-200 dark:border-white/10">
