...
const MemberCRMDialog: React.FC<MemberCRMDialogProps> = ({ member, isOpen, onOpenChange }) => {
    ...
    return (
        ...
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-4xl rounded-3xl p-0 overflow-hidden border-none shadow-2xl flex flex-col h-[85vh] bg-card dark:bg-slate-950">
                <div className="flex flex-1 overflow-hidden">
                    ...
                    <div className="w-1/3 bg-slate-50/50 dark:bg-slate-900 border-r border-slate-100 dark:border-white/5 p-8 flex flex-col pt-10">
                        <h3 className="text-xl font-bold text-foreground dark:text-white mb-8 tracking-tight">Member Profile</h3>
                        ...
                        <div className="flex flex-col items-center mb-10 relative">
                            ...
                            <h2 className="text-2xl font-bold text-foreground dark:text-white text-center mt-6 tracking-tight leading-tight">
                                {member?.profiles?.first_name} {member?.profiles?.last_name}
                            </h2>
                            <div className="flex flex-wrap items-center justify-center gap-2 mt-3">
                                <Badge className="bg-indigo-600/10 text-indigo-700 border-none font-bold text-[8px] uppercase tracking-[0.2em] px-3 py-1 scale-110">
                                     {member?.membership_type || 'GUEST'}
                                </Badge>
                                {member?.profiles?.role && member.profiles.role !== 'regular_user' && (
                                    <Badge className="bg-amber-600/10 text-amber-700 border-none font-bold text-[8px] uppercase tracking-[0.2em] px-3 py-1 scale-110">
                                         STAFF
                                    </Badge>
                                )}
                            </div>
                        ...
                        <div className="space-y-6 flex-1 overflow-y-auto pr-2 custom-scrollbar">
                            <div className="space-y-4">
                                <div className="flex items-center gap-3 text-slate-500">
                                    <div className="w-8 h-8 rounded-xl bg-card dark:bg-slate-800 flex items-center justify-center shadow-sm"><Phone className="h-3.5 w-3.5" /></div>
                                    <div>
                                        <p className="text-[10px] font-bold uppercase text-slate-400">Phone</p>
                                        <p className="text-xs font-bold text-foreground dark:text-white">{member?.profiles?.phone || 'Not recorded'}</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-3 text-slate-500">
                                    <div className="w-8 h-8 rounded-xl bg-card dark:bg-slate-800 flex items-center justify-center shadow-sm"><Mail className="h-3.5 w-3.5" /></div>
                                    <div className="truncate">
                                        <p className="text-[10px] font-bold uppercase text-slate-400">Email</p>
                                        <p className="text-xs font-bold text-foreground dark:text-white truncate">{member?.profiles?.email || 'No email registered'}</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-3 text-slate-500">
                                    <div className="w-8 h-8 rounded-xl bg-card dark:bg-slate-800 flex items-center justify-center shadow-sm"><MapPin className="h-3.5 w-3.5" /></div>
                                    <div>
                                        <p className="text-[10px] font-bold uppercase text-slate-400">Address</p>
                                        <p className="text-xs font-bold text-foreground dark:text-white line-clamp-1">{member?.profiles?.address || 'No address recorded'}</p>
                                    </div>
                                </div>
                            </div>

                            <Separator className="bg-slate-100 dark:bg-card/5" />
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="text-[10px] font-bold uppercase text-slate-400">Member Since</p>
                                        <p className="text-xs font-bold text-foreground dark:text-white">{member?.joined_at ? format(new Date(member.joined_at), 'MMMM d, yyyy') : 'Recently'}</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="text-[10px] font-bold uppercase text-slate-400">Departments & Roles</p>
                                        <div className="flex flex-wrap gap-1 mt-1">
                                            {assignments && assignments.length > 0 ? assignments.map((a: any) => (
                                                <Badge key={a.id} variant="outline" className="text-[9px] h-5 px-2 bg-card dark:bg-slate-800 border-slate-100 dark:border-white/5 font-bold uppercase">{a.ministry?.name}</Badge>
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="text-[10px] font-bold uppercase text-slate-400">Small Groups</p>
                            ...
                            <Button onClick={contactActions.messageSMS} className="bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl h-14 font-bold shadow-lg shadow-indigo-100 dark:shadow-none flex flex-col gap-0.5"><MessageSquare className="h-4 w-4" /> <span className="text-[10px] uppercase">Text</span></Button>
                                ...
                                <div className="space-y-4 col-span-2 pt-4">
                                    <Button 
                                        ...
                                        className={`w-full bg-slate-900 border-none text-white rounded-3xl h-14 font-bold shadow-2xl flex items-center justify-center gap-3 transition-all hover:scale-[1.02] active:scale-95 ${
                                            member?.journey_stage === 'member' ? 'opacity-0 pointer-events-none hidden' : ''
                                        }`}
                                    >
                                        <div className="w-8 h-8 rounded-xl bg-card/10 flex items-center justify-center">
                                    ...
                                    {member?.journey_stage !== 'member' && (
                                        <p className="text-[8px] font-bold text-slate-400 uppercase tracking-[0.2em] text-center px-6 leading-relaxed opacity-60">
                    ...
                    <div className="flex-1 flex flex-col pt-10">
                        <Tabs defaultValue="overview" className="flex-1 flex flex-col px-10">
                            <div className="flex items-center justify-between mb-8">
                                <TabsList className="bg-slate-100/80 dark:bg-card/5 p-1 rounded-2xl h-14">
                                    <TabsTrigger value="overview" className="rounded-xl px-6 font-bold text-xs uppercase tracking-widest data-[state=active]:bg-card dark:data-[state=active]:bg-slate-800 data-[state=active]:shadow-lg transition-all">Timeline</TabsTrigger>
                                    <TabsTrigger value="journey" className="rounded-xl px-6 font-bold text-xs uppercase tracking-widest data-[state=active]:bg-card dark:data-[state=active]:bg-slate-800 data-[state=active]:shadow-lg transition-all">Journey</TabsTrigger>
                                    <TabsTrigger value="giving" className="rounded-xl px-6 font-bold text-xs uppercase tracking-widest data-[state=active]:bg-card dark:data-[state=active]:bg-slate-800 data-[state=active]:shadow-lg transition-all">Giving</TabsTrigger>
                                    <TabsTrigger value="tasks" className="rounded-xl px-6 font-bold text-xs uppercase tracking-widest data-[state=active]:bg-card dark:data-[state=active]:bg-slate-800 data-[state=active]:shadow-lg transition-all">Planning & Care</TabsTrigger>
                                </TabsList>
                            ...
                            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar relative">
                                <TabsContent value="overview" className="mt-0 space-y-8 pb-10">
                                    <div className="flex items-center justify-between px-2">
                                        <h4 className="text-sm font-bold text-foreground dark:text-white uppercase tracking-[0.2em]">Interaction History</h4>
                                    ...
                                    <div className="relative space-y-8 before:absolute before:inset-0 before:ml-5 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-indigo-100 before:via-slate-50 before:to-transparent dark:before:from-white/10">
                                        {interactions && interactions.length > 0 ? interactions.map((interaction, idx) => (
                                            ...
                                            >
                                                <div className="absolute left-0 top-0 mt-1 flex h-10 w-10 items-center justify-center rounded-2xl bg-card dark:bg-slate-800 shadow-xl shadow-slate-200/50 dark:shadow-none ring-4 ring-white dark:ring-slate-950 z-10 transition-transform hover:scale-110">
                                                ...
                                                <div className="flex-1">
                                                    <div className="flex items-center justify-between mb-2">
                                                        <h5 className="font-bold text-foreground dark:text-white text-[11px] uppercase tracking-wider">
                                                    ...
                                                    <Card className="p-4 rounded-2xl border-none shadow-sm bg-slate-50/50 dark:bg-card/5 group hover:bg-card dark:hover:bg-card/10 transition-all">
                                                        ...
                                                        {(interaction as any).author && (
                                                            <div className="mt-3 pt-3 border-t border-slate-100 dark:border-white/5 flex items-center gap-2">
                                                                <div className="w-5 h-5 rounded-full bg-indigo-100 flex items-center justify-center text-[8px] font-bold text-indigo-600 uppercase">
                                                                    {(interaction as any).author.first_name?.[0]}
                                                                </div>
                                                                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Added by {(interaction as any).author.first_name}</span>
                                                            </div>
                                                        )}
                                                    </Card>
                                                </div>
                                            ...
                                            <div className="py-24 text-center opacity-20">
                                                ...
                                                <p className="font-bold uppercase tracking-[0.3em] text-xs">Waiting for first connection</p>
                                            </div>
                                        )}
                                    </div>
                                </TabsContent>
                                ...
                                <TabsContent value="journey" className="mt-0 space-y-8 pb-10">
                                     <div className="p-10 bg-indigo-600 rounded-[3rem] text-white relative overflow-hidden group shadow-2xl shadow-indigo-200 dark:shadow-none min-h-[220px] flex flex-col justify-center">
                                        ...
                                        <div className="relative z-10">
                                            <div className="flex items-center gap-4 mb-4">
                                                <p className="text-[10px] font-bold uppercase tracking-[0.3em] opacity-60">Journey Momentum</p>
                                                <div className="h-0.5 flex-1 bg-card/20" />
                                            </div>
                                            <div className="flex items-center justify-between gap-6">
                                                <h3 className="text-5xl font-bold tracking-tighter uppercase drop-shadow-sm">{member?.journey_stage?.replace(/_/g, ' ') || 'DISCOVERY'}</h3>
                                                
                                                <DropdownMenu>
                                                    <DropdownMenuTrigger asChild>
                                                        <Button variant="outline" className="bg-card/10 border-white/20 text-white rounded-2xl font-bold text-[10px] uppercase tracking-widest hover:bg-card hover:text-indigo-600 h-10 px-6 backdrop-blur-md">
                                                            Change Stage
                                                        </Button>
                                                    </DropdownMenuTrigger>
                                                    <DropdownMenuContent align="end" className="rounded-2xl border-none shadow-2xl p-2 min-w-[220px]">
                                                        {['initial_visit', 'followed_up', 'connected', 'member'].map(stage => (
                                                            <DropdownMenuItem 
                                                                key={stage} 
                                                                className="font-bold text-[10px] uppercase p-4 rounded-xl cursor-pointer hover:bg-slate-50 transition-colors"
                                                                onClick={() => {
                                            ...
                                            <div className="flex gap-3 mt-8">
                                                <Badge className="bg-card/20 text-white border-none font-bold text-[9px] uppercase tracking-widest h-8 px-5 rounded-full">VIP TRACKING</Badge>
                                                <Badge className="bg-card/20 text-white border-none font-bold text-[9px] uppercase tracking-widest h-8 px-5 rounded-full">COMMUNITY GROWTH</Badge>
                                            </div>
                                        </div>
                                     </div>
                                     ...
                                     <div className="space-y-6">
                                        <div className="flex items-center justify-between px-2">
                                            <h4 className="text-sm font-bold text-foreground dark:text-white uppercase tracking-[0.2em]">Spiritual Milestones</h4>
                                            <Button 
                                                size="sm" 
                                                variant="ghost" 
                                                onClick={() => setIsMilestoneDialogOpen(true)}
                                                className="h-9 px-4 rounded-xl text-indigo-600 font-bold text-[10px] uppercase tracking-widest hover:bg-indigo-50"
                                            >
                                        ...
                                        <div className="grid grid-cols-1 gap-4">
                                            {milestones?.map(m => (
                                                <Card key={m.id} className="flex items-center gap-6 p-6 bg-slate-50 dark:bg-slate-900/50 rounded-[2rem] border-none hover:shadow-md transition-all group">
                                                    <div className="w-14 h-14 rounded-2xl bg-card dark:bg-slate-800 flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform">
                                                        <Trophy className="h-6 w-6 text-indigo-600" />
                                                    </div>
                                                    <div className="flex-1">
                                                        <h5 className="font-bold text-foreground dark:text-white uppercase tracking-tight text-xs">{m.milestone_type.replace(/_/g, ' ')}</h5>
                                                    ...
                                                    {m.notes && <Badge className="bg-emerald-50 text-emerald-600 border-none text-[8px] font-bold uppercase tracking-widest px-3">VERIFIED</Badge>}
                                                </Card>
                                            ))}
                                            {(!milestones || milestones.length === 0) && (
                                                <div className="p-16 border-4 border-dashed border-slate-50 rounded-[3rem] text-center text-slate-300 text-[10px] font-bold uppercase tracking-[0.3em]">No milestones verified yet</div>
                                            )}
                                        </div>
                                     </div>
                                </TabsContent>
                                ...
                                <TabsContent value="giving" className="mt-0 space-y-8 pb-10">
                                    <div className="grid grid-cols-2 gap-6">
                                        <Card className="p-8 rounded-[2.5rem] bg-emerald-500 text-white border-none flex flex-col justify-between h-40 shadow-xl shadow-emerald-100 dark:shadow-none relative overflow-hidden">
                                            ...
                                            <p className="text-[10px] font-bold uppercase tracking-[0.3em] opacity-60">Heart for the Kingdom</p>
                                            <h4 className="text-4xl font-bold tracking-tighter">${donations?.reduce((acc: number, d: any) => acc + Number(d.amount), 0).toLocaleString()}</h4>
                                        </Card>
                                        <Card className="p-8 rounded-[2.5rem] bg-indigo-500 text-white border-none flex flex-col justify-between h-40 shadow-xl shadow-indigo-100 dark:shadow-none relative overflow-hidden">
                                            ...
                                            <p className="text-[10px] font-bold uppercase tracking-[0.3em] opacity-60">Consistency</p>
                                            <h4 className="text-4xl font-bold tracking-tighter">85%</h4>
                                        </Card>
                                    </div>

                                    <div className="space-y-6">
                                        <div className="flex items-center justify-between px-2">
                                            <h4 className="text-sm font-bold text-foreground dark:text-white uppercase tracking-[0.2em]">Donation Feed</h4>
                                        ...
                                        <div className="space-y-4">
                                            {donations?.map((donation: any, idx: number) => (
                                                ...
                                                >
                                                    <Card className="flex items-center justify-between p-6 bg-card dark:bg-slate-900 border-none shadow-sm hover:shadow-md transition-all">
                                                        <div className="flex items-center gap-5">
                                                            ...
                                                            <div>
                                                                <h5 className="font-bold text-foreground dark:text-white uppercase tracking-tight text-xs">{donation.ministry?.name || 'General Offering'}</h5>
                                                        ...
                                                        <span className="font-bold text-emerald-600 text-lg">${Number(donation.amount).toLocaleString()}</span>
                                                    </Card>
                                                </motion.div>
                                            ))}
                                        </div>
                                ...
                                <TabsContent value="tasks" className="mt-0 space-y-6 pb-10">
                                    <div className="flex items-center justify-between px-2">
                                        <h4 className="text-sm font-bold text-foreground dark:text-white uppercase tracking-[0.2em]">Care Follow-ups</h4>
                                    ...
                                    <div className="grid grid-cols-1 gap-4">
                                        <Card className="p-6 rounded-[2rem] bg-slate-50 border-dashed border-2 border-slate-200">
                                            <div className="space-y-4">
                                                <div className="space-y-2">
                                                    <Label className="text-[10px] font-bold uppercase text-slate-400 px-1">Quick Add Task</Label>
                                                    <div className="flex gap-2">
                                                        <input 
                                                            placeholder="Follow up on prayer request..." 
                                                            value={newTaskTitle}
                                                            onChange={e => setNewTaskTitle(e.target.value)}
                                                            className="flex-1 h-12 bg-card dark:bg-slate-900 border-none rounded-xl px-4 text-sm font-bold shadow-sm focus:ring-2 focus:ring-indigo-500 transition-all"
                                                        />
                                                        <Select value={newTaskPriority} onValueChange={(v: any) => setNewTaskPriority(v)}>
                                                            <SelectTrigger className="w-24 h-12 bg-card dark:bg-slate-900 border-none rounded-xl font-bold text-[10px] uppercase shadow-sm">
                                                                <SelectValue />
                                                            </SelectTrigger>
                                        ...
                                        {memberTasks.map(task => (
                                            <Card key={task.id} className="p-6 rounded-3xl bg-card dark:bg-slate-900 border-none shadow-sm group hover:shadow-xl transition-all border border-transparent hover:border-indigo-100">
                                                <div className="flex justify-between items-start">
                                                    <div className="flex gap-4">
                                                        ...
                                                        <div>
                                                            <h5 className="font-bold text-foreground dark:text-white uppercase tracking-tight text-xs">{task.title}</h5>
                                                            <Badge className={cn(
                                                                "mt-1 text-[8px] font-bold uppercase tracking-widest px-2 py-0 border-none",
                                                    ...
                                                    <Button 
                                                        ...
                                                        className={cn(
                                                            "rounded-xl font-bold text-[9px] uppercase tracking-widest h-9 px-4",
                                        ...
                                        {memberTasks.length === 0 && !newTaskTitle && (
                                            <div className="py-20 text-center opacity-20">
                                                ...
                                                <p className="font-bold uppercase tracking-[0.3em] text-[10px]">No active care items</p>
                                            </div>
                                        )}
                                    </div>
                                </TabsContent>
                                ...
                                <div className="sticky bottom-0 left-0 right-0 bg-gradient-to-t from-white dark:from-slate-950 via-white dark:via-slate-950 to-transparent pt-10 pb-4 z-20">
                                    <div className="relative">
                                        <Textarea 
                                            ...
                                            className="min-h-[100px] rounded-3xl bg-slate-50/50 dark:bg-card/5 border-2 border-slate-100 dark:border-white/10 p-5 pr-20 font-medium text-sm focus:ring-4 focus:ring-indigo-500/10 transition-all resize-none"
                                        />
                                        <Button 
                                            onClick={() => {
        ...
        <Dialog open={isMilestoneDialogOpen} onOpenChange={setIsMilestoneDialogOpen}>
            <DialogContent className="max-w-md rounded-[2.5rem] border-none shadow-2xl p-0 overflow-hidden bg-card dark:bg-slate-950">
                <div className="bg-indigo-600 p-8 text-white relative overflow-hidden">
                    ...
                    <DialogTitle className="text-2xl font-bold uppercase tracking-tight relative z-10">Record Milestone</DialogTitle>
                ...
                <div className="p-8 space-y-6">
                    <div className="space-y-2">
                        <Label className="text-[10px] font-bold uppercase text-slate-400 tracking-widest px-1">Event Type</Label>
                    ...
                    <div className="space-y-2">
                        <Label className="text-[10px] font-bold uppercase text-slate-400 tracking-widest px-1">Date Attained</Label>
                        <input 
                            type="date"
                            value={milestoneDate}
                    ...
                    <Button 
                        className="w-full h-14 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-bold shadow-xl active:scale-95 transition-all mt-4"
