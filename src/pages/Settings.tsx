      ...
      <div className="space-y-12 max-w-6xl mx-auto py-8 px-4 sm:px-6">
        ...
        >
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3 ml-1">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Configure Experience</p>
              <div className="h-1 w-1 rounded-full bg-slate-300" />
              <p className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.3em]">Personalized Terminal</p>
            </div>
          </div>
          
          <div className="flex items-center gap-2 p-1.5 bg-slate-100 dark:bg-white/5 rounded-2xl border border-slate-200 dark:border-white/5">
             <div className="px-4 py-2 bg-white dark:bg-white/10 rounded-xl shadow-sm border border-white/50 dark:border-white/5">
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-600 dark:text-slate-300">Active Node: {activeTab}</span>
             </div>
          </div>
        </motion.div>
        ...
        <Tabs defaultValue="general" value={activeTab} onValueChange={setActiveTab} className="w-full space-y-10">
          <TabsList className="flex flex-wrap h-auto bg-transparent border-none gap-3 p-0 md:justify-start">
            {tabItems.map((item) => (
              <TabsTrigger 
                ...
                value={item.value} 
                className={cn(
                  "flex items-center gap-3 px-8 h-14 rounded-[1.5rem] transition-all duration-300 font-black uppercase tracking-widest text-[10px] border-none shadow-none",
                  activeTab === item.value 
                    ? "bg-indigo-600 text-white shadow-xl shadow-indigo-200 dark:shadow-indigo-500/20 translate-y-[-2px]" 
                    : "bg-white dark:bg-slate-900 text-slate-400 hover:bg-indigo-50 dark:hover:bg-white/5 hover:text-indigo-600"
                )}
              >
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <Card className="bg-white dark:bg-slate-900 rounded-[2.5rem] border-none shadow-[LRB] dark:shadow-black/60 shadow-slate-200/50 overflow-hidden group">
        ...
        <motion.div 
          ...
          className="flex flex-col sm:flex-row items-center justify-between gap-6 p-10 bg-slate-50 dark:bg-white/5 rounded-[2.5rem] border border-slate-100 dark:border-white/5"
        >
          <div className="flex items-center gap-4">
             ...
             <div>
                <p className="text-[10px] font-black text-slate-900 dark:text-white uppercase tracking-widest">Global Encryption Active</p>
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mt-0.5">All settings are synchronized across devices</p>
             </div>
          </div>
          <button className="flex items-center gap-2 group">
             <span className="text-[10px] font-black text-slate-400 group-hover:text-indigo-600 transition-colors uppercase tracking-widest">Documentation</span>
